...
var repackCommand = cli.Command{
	...
	ArgsUsage: `--image <image-path>[:<new-tag>] --bundle <bundle-path>
...
"latest"), and "<bundle-path>" is the bundle from which to generate the
required layers.

The "<image-path>" MUST be the same image that was used to create
"<bundle-path>" (using umoci-unpack(1)). Otherwise umoci will not be able to
modify the original manifest to add the diff layer.
