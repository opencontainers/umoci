...
@test "umoci raw mtree-validate [unpacked rootfs]" {
	...
	if [[ "$IS_ROOTLESS" == 0 ]]; then
		sane_run gomtree validate -K sha256digest -f "$BUNDLE"/sha256_*.mtree -p "$ROOTFS"
		[ "$status" -eq 0 ]
	fi
...
@test "umoci raw mtree-validate [basic manifest]" {
	...
	[ "$status" -eq 0 ]
	sane_run gomtree validate -p "$rootfs" -f "$manifest"
	[ "$status" -eq 0 ]
